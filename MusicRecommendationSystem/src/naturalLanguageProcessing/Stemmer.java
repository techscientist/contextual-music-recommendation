...
public class Stemmer implements Function<Word,Word> {
  ...
  */
  ...
  /*
   * @author: Andrea Rossi
   * @input: tokens è un'arrayList di stringhe ottenute da precedente tokenizzazione di un testo
   * @output: un'arrayList di stems parallelo a tokens
   * 
   * usando la libreria di Stanford e la classe Stemmer preesistente
   * (ottenuta qui:
   *  richiedo l'esecuzione di stemming algoritmico su token preesistenti
   *  per ottenere i rispettivi stems 
   *  (NOTA: sia in input che in output potrebbero esserci doppioni) */
	public ArrayList<String> porterStem(ArrayList<String> tokens) {
		
		Stemmer stemmer = new Stemmer();
		ArrayList<String> stems = new ArrayList<String>(tokens.size());
		
		for(int i = 0; i < tokens.size(); i++) {
			stems.add(stemmer.stem(tokens.get(i)));
		}
		return stems;
	}
}
